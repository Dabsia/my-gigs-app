...
const Progress = () => {
  ...
  const savedDurationRef = useRef<number>(0); // Duration before current session
        ...
        if (data.success && data.data) {
          ...
          if (timerData.entry) {
            if (timerData.entry.status === "running") {
              ...
              // Set saved duration from previous sessions
              savedDurationRef.current = timerData.currentDuration || 0;

              // Tracked time = saved duration + elapsed time
              setTrackedTime(savedDurationRef.current + elapsedSeconds);
              setTimerStatus("running");
              startLocalTimer();
            } else if (timerData.entry.status === "paused") {
              // Timer is paused
              setTimerStatus("paused");
              const savedDuration = timerData.currentDuration || 0;
              savedDurationRef.current = savedDuration;
              setTrackedTime(savedDuration);
  ...
  const startLocalTimer = () => {
    ...
    intervalRef.current = setInterval(() => {
      setTrackedTime((prev) => {
        // If we have a start time, calculate from it
        if (startTimeRef.current) {
          const now = new Date();
          const elapsedSeconds = Math.floor(
            (now.getTime() - startTimeRef.current.getTime()) / 1000
          );
          return savedDurationRef.current + elapsedSeconds;
        }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          savedDurationRef.current = trackedTime;
          startTimeRef.current = null;
  ...
  const resumeTimeTracking = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
          // Set current time as start time for resumed timer
      ...
      >
        ...
        {/* <View className="py-6 border-b border-gray-100">
          ...
          {timeSummary ? (
              ...
              <View className="bg-secondary rounded-lg p-4">
                <View className="flex-row items-center justify-between">
                  <View>
                    ...
                    <Text className="text-white text-2xl font-bold mt-1">
                      {formatMinutes(timeSummary.project?.totalTimeSpent || 0)}
                    </Text>
                  </View>
                ...
                <View className="mt-3 pt-3 border-t border-white/30">
                  <Text className="text-white/80 text-sm">
                    Project: {timeSummary.project?.title || "Untitled"}
                  </Text>
                  <Text className="text-white/80 text-sm mt-1">
                    Hourly Rate:{" "}
                    {formatCurrency(timeSummary.project?.hourlyRate || 0)}
                  </Text>
                </View>
              </View>

              {/* Quick Stats */}
              <View className="bg-[#F9FAFC] rounded-lg border border-gray-200 p-4">
                ...
                <View className="grid grid-cols-2 gap-3">
                  <View className="bg-white p-3 rounded-lg border border-gray-100">
                    ...
                    <Text className="text-lg font-bold text-gray-900 mt-1">
                      {timeSummary.summary.totalEntries}
                    </Text>
                  </View>
                  ...
                  <View className="bg-white p-3 rounded-lg border border-gray-100">
                    ...
                    <Text className="text-lg font-bold text-gray-900 mt-1">
                      {timeSummary.summary.totalHours}
                    </Text>
                  </View>
                  ...
                  <View className="bg-white p-3 rounded-lg border border-gray-100">
                    ...
                    <Text className="text-lg font-bold text-green-600 mt-1">
                      {timeSummary.summary.billableHours}
                    </Text>
                  </View>
                  ...
                  <View className="bg-white p-3 rounded-lg border border-gray-100">
                    ...
                    <Text className="text-lg font-bold text-gray-900 mt-1">
                      {timeSummary.summary.averageEntryDuration}m
                    </Text>
                  </View>
                </View>
              </View>
                ...
                <View className="bg-white rounded-lg p-4">
                  <View className="flex-row justify-between items-center mb-3">
                    ...
                    <Text className="text-xl font-bold text-green-600">
                      {formatCurrency(
                        parseFloat(timeSummary.summary.billableAmount)
                      )}
                    </Text>
                  </View>
                  ...
                  <View className="flex-row justify-between items-center">
                    ...
                    <Text className="text-lg font-semiBold text-gray-900">
                      {formatCurrency(timeSummary.summary.averageRate)}
                    </Text>
                  </View>
                  ...
                  <View className="mt-4 pt-3 border-t border-gray-200">
                    ...
                    <Text className="text-base font-semiBold text-gray-900">
                      {formatMinutes(timeSummary.summary.nonBillableMinutes)}
                    </Text>
                  </View>
                </View>
              </View>
                          ...
                          >
                            ...
                            <View className="flex-row justify-between items-center">
                              ...
                              <View className="flex-row items-center gap-2">
                                <Text className="text-xs font-semiBold text-gray-900 bg-gray-100 px-2 py-1 rounded">
                                  {entry.duration}m
                                </Text>
                                {entry.billable ? (
                                  <Text className="text-xs font-semiBold text-green-600 bg-green-50 px-2 py-1 rounded">
                                    Billable
                                  </Text>
                                ) : (
                                  <Text className="text-xs font-semiBold text-gray-600 bg-gray-100 px-2 py-1 rounded">
                                    Non-billable
                                  </Text>
                                )}
                              </View>
            ...
            <View className="bg-[#F9FAFC] rounded-lg border border-gray-200 p-6">
              <View className="items-center">
                ...
                <Text className="text-gray-500 text-sm text-center mt-1">
                  Start tracking time to see your summary here.
                </Text>
              </View>
            </View>
          )}
        ...
        <View className="py-4 border-b border-gray-100">
          ...
          {/* Progress Bar */}
          <View className="w-full bg-gray-200 rounded-full h-4 mb-6">
            <View
